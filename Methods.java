...
public class Methods {
    ...
    public void shipNewPackage() {
        ...
        */
        ...
        MongoCollection<Document> col = database.getCollection("Package");
    ...
    public void updateCurrentLocation() { //**This works**
        ...
        try {
         ...
         //This retrieves the entire document that matches the tracking number (Using _id) 
            Document myDoc = (Document) collectionPackage.find(eq("trackingNum", packageToSearch)).first();
            ...
            //get the current state
             String currState = (String) (myDoc.get("currentState"));
    ...
    public double calculateShippingCost() {
        ...
        try {
            //This retrieves the zip code document that matches the sender's zip code 
           
            Document sendZip = (Document) collectionZipCode.find(eq("Zipcode", senderZipcode)).first();
            if (sendZip == null) {
                System.out.println("I'm sorry, but we do not have the sender's zip code in our system.");
            }
            ...
            if (toZip == null) {
                System.out.println("I'm sorry, but we do not have the recipient's zip code in our system.");
            }
    ...
    public void trackPackageByTrackingNumber() { //****this works ********
        ...
        try {
            ...
            if (currentCity == null) { //if the current location is null
                System.out.println("Im sorry, but the current city location of the package is not available.");
            }
    ...
    // Returns Spherical distance in miles given the latitude 
    // and longitude of two points (depends on constant RADIUS)
    public double distance2(double lat1, double long1, double lat2, double long2) {
     ...
     public byte[] getSalt(String adminLogin) throws Exception {
    ...
    MongoCollection<Document> col = database.getCollection("Administrator");
     ...
     byte[] password = new byte[32];
      ...
      //This retrieves the entire document that matches login 
       Document myDoc = (Document) collectionEE.find(eq("login", login)).first();
